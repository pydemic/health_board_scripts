defmodule HealthBoard.Scripts.Morbidities.Mortalities.Parser do
  ...
  @columns [
    {"DTOBITO", :year, :required},
    {"DTOBITO", :week_year, :optional},
    {"DTOBITO", :week, :optional},
  ...
  defp parse_value(value, type) do
    case type do
      ...
      :week_year -> parse_week_year(value)
      :week -> parse_week(value)
    end
  rescue
    _error -> nil
  end
  ...
  defp parse_week_year(value) do
    if String.length(value) == 8 do
      day = String.to_integer(String.slice(value, 0, 2))
      month = String.to_integer(String.slice(value, 2, 2))
      year = String.to_integer(String.slice(value, 4, 4))
      {year, _week} = :calendar.iso_week_number({year, month, day})
      year
    else
      nil
    end
